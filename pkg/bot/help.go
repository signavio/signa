...
func (b *Bot) showHelp(c *Cmd, help *customCommand) {
	if help.Description != "" {
		b.Handler.Response(c.Channel, fmt.Sprintf(helpDescripton, help.Description), c.User)
	}
	b.Handler.Response(c.Channel, fmt.Sprintf(helpUsage, CmdPrefix, c.Command, help.ExampleArgs), c.User)
}
...
func (b *Bot) showAvailabeCommands(channel string, sender *User) {
	...
	b.Handler.Response(channel, fmt.Sprintf(helpAboutCommand, CmdPrefix), sender)
	b.Handler.Response(channel, fmt.Sprintf(availableCommands, strings.Join(cmds, ", ")), sender)
}
